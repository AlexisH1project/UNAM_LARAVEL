...
class DDSCHController extends Controller
{
    ...
    public function EnviarFomope(Request $request){
        ...
        $Documents = $DocumentoAdd."_".$Docs;
        $mytime = Carbon::now();
        $mytime->setTimezone('GMT-6'); 
        $arrayDoc = explode("_",$Documents);
        $hora = str_replace ( ":", '',$mytime->toTimeString()); 
        $fecha = str_replace ( "-", '',$mytime->toDateString()); 
        if(count($arrayDoc) < 3){ //el arreglo guarda el nombre del doc, uno en blanco por _ y el ultimo que es por defaul
            DB::insert('insert into fomope (color_estado,usuario_name,unidad,rfc,curp,apellido_1,apellido_2,nombre,fechaIngreso,quincenaAplicada,anio,vigenciaDel,vigenciaAl) values (?,?,?,?,?,?,?,?,?,?,?,?,?)',
            ["amarillo0 ",Auth::user()->usuario, $unidad, $rfc, $curp, $apellido1, $apellido2, $nombre, $fechaIngreso,  $quincena,  $mytime->weekYear(), $del2, $al3]);    
        }
        $rowsDoc = DB::table('m1ct_documentos')->select('*')->where('nombre_documento','=', $DocumentoAdd)->get();
        $contador = 0;
        $abreviaturaDoc = [];
        foreach ($rowsDoc as $registro) {
            $abreviaturaDoc[$contador] = $registro->documentos;
            $contador++;
        }
...
'fechaAutorizacion' => $user_Fecha,
        ...
        // $concatenarNombreC = strtoupper($request->get('rfc')."_". $abreviaturaDoc[0]."_". $request->get('apellido1')."_". $request->get('apellido2')."_". $request->get('nombre')."_".$fecha.$hora."_". $request->get('movFecha')."_.".$extencion[1]); // strtoupper: lo pasa a mayuscula todo 
        ...
        // Storage::move($path, 'DOCUMENTOS_MOV/'.$request->get('documentoSelect').'/'.$concatenarNombreC);
         //     **************************el update
        $obtenerRow = DB::table('fomope')
        ->where('color_estado', 'amarillo0')
        ->where('usuario_name', Auth::user()->usuario)
        ->where('unidad', $unidad)
        ->where('rfc', $rfc)
        ->where('curp', $curp)
        ->where('apellido_1', $apellido1)
        ->where('apellido_2', $apellido2)
        ->where('nombre', $nombre)
        ->where('fechaIngreso', $fechaIngreso)
        ->where('vigenciaDel', $del2)
        ->where('vigenciaAl', $al3)
        ->where('quincenaAplicada', $quincena)
        ->where('anio', $mytime->weekYear())
        ->where('fechaValidacionPersonal', $mytime->toDateString())
        ->first();
       echo $obtenerRow->id_movimiento;
